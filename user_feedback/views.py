...
def send_bug_report_emails(request, data):
    mail_admins(
        ...
        message= "A bug has been reported on %s's %s. The user has written the following: '%s' There have been %d other bugs reported in the last 10 days at this url." % (request.get_host(), data['url'][1:-1], data['text'], Feedback.objects.filter(type=1, url=data['url'], created_on__lte=datetime.datetime.today(), created_on__gt=datetime.datetime.today()-datetime.timedelta(days=10)).count()-1),
        fail_silently=False,
    )
